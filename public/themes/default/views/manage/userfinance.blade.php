...
<div class="table-responsive">
    <table id="sample-table-1" class="table table-striped table-bordered table-hover">
        ...
        <tr>
            ...
            <td>
                @if($item->action == 1)发布任务@elseif($item->action == 2)接受任务@elseif($item->action == 3)用户充值@elseif($item->action == 4)用户提现
                @elseif($item->action == 5)购买增值服务@elseif($item->action == 6)购买作品@elseif($item->action == 7)任务失败退款
                @elseif($item->action == 8)提现失败退款@elseif($item->action == 9)出售作品@elseif($item->action == 10)维权退款
                @elseif($item->action == 11)推荐到威客商城失败退款
                @endif
            </td>
            <td >{!! $item->name !!}</td>
            <td>￥{!! $item->cash !!}元</td>
