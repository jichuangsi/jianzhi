...
<h3 class="header smaller lighter blue mg-bottom20 mg-top12">任务分类详情</h3>

<div class="g-backrealdetails clearfix bor-border">
                <!-- PAGE CONTENT BEGINS -->
                <form class="form-inline" role="form" method="post" action="/manage/taskTypeInfo" enctype="multipart/form-data">
                    {!!  csrf_field() !!}
                            <!-- #section:elements.form -->
                    <div class="bankAuth-bottom clearfix col-xs-12">
                        <p class="col-sm-1 text-right" for="form-field-1"> 当前分类： </p>

                        <p class="col-sm-4" for="form-field-1"> {!! $taskType['name'] !!} </p>
                        <input name="id" type="hidden" value="{!! $taskType['id'] !!}">
                        <input name="pid" type="hidden" value="{!! $taskType['pid'] !!}">
                    </div>

                    <div class="bankAuth-bottom clearfix col-xs-12">
                        <p class="col-sm-1 text-right" for="form-field-1"> 所属分类： </p>

                        <p class="col-sm-4 no-padding-right" for="form-field-1"> @if($taskType['pid'] == 0)全部@else{!! $parent_taskType['name'] !!}@endif </p>
                    </div>
                    <div class="bankAuth-bottom clearfix col-xs-12">
                        ...
                        <div class="col-sm-4">
                            <div class="memberdiv pull-left">
                                <div class="position-relative">
                                    ...
                                    @if($taskType['pic'])
                                    <img src="{!! url($taskType['pic']) !!} " width="152" height="126">
                                    @endif
                                </div>
                            </div>
                        </div>
                        <p class="col-sm-3 no-padding-right text-left red" for="form-field-1"> 该图标适用于APP端，与PC端无关联 </p>
                    ...
                    <div class="col-xs-12">
                        <div class="clearfix row bg-backf5 padding20 mg-margin12">
                            <div class="col-xs-12">
                                ...
                                <div class="btn btn-primary btn-sm" onclick="window.location.href = '{!! url('manage/taskType') !!}'">返回</div>
                                </div>
                            </div>
                        </div>
                    </div>
