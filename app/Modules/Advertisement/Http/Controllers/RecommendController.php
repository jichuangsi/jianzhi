...
class RecommendController extends ManageController
{
    ...
    public function addRecommend(Request $request){
        ...
        if(!in_array($data['type'],['shop','work','server'])){
            if(empty($file)){
        ...
        if((!in_array($data['type'],['shop','work','server'])) || (in_array($data['type'],['shop','work','server']) && !empty($file))){
            //上传文件
    ...
    public function updateRecommend($id){
        ...
        switch($serviceInfo[0]->type){
            ...
            case 'task':
                ...
                break;
            case 'shop':
                $userInfo = ShopModel::where('status',1)->select('id','shop_name')->get();
                break;
            case 'work':
                $userInfo = TaskModel::where(['status' => 1,'type' => 1,'is_delete' => 0])->select('id','title')->get();
                break;
            case 'server':
                $userInfo = TaskModel::where(['status' => 1,'type' => 2,'is_delete' => 0])->select('id','title')->get();
                break;
        }
        $domain = \CommonClass::domain();
        $view = [
    ...
    public function getReInfo(Request $request){
        ...
        switch($type){
            ...
            case 'task':
                ...
                break;
            case 'shop':
                $shopInfo = ShopModel::where('status',1)->select('id','shop_name')->get()->toArray();
                if(count($shopInfo)){
                ...
                break;
            case 'work':
                $workInfo = GoodsModel::where(['type' => 1,'status' => 1,'is_delete' => 0])->select('id','title')->get()->toArray();
                if(count($workInfo)){
                ...
                break;
            case 'server':
                $serviceInfo = GoodsModel::where(['type' => 2,'status' => 1,'is_delete' => 0])->select('id','title')->get()->toArray();
